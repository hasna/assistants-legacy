...
// src/cli/index.ts
import { existsSync as existsSync6, mkdirSync as mkdirSync5, writeFileSync as writeFileSync6, readFileSync as readFileSync4 } from "fs";
import { join as join6, resolve, extname, basename } from "path";
...
messagesCmd.command("list").description("List messages in your mailbox").option("-n, --max <number>", "Maximum messages to return", "10").option("-q, --query <query>", 'Gmail search query (e.g., "is:unread", "from:someone@example.com")').option("-l, --label <label>", "Filter by label ID").action(async (opts) => {
  try {
    ...
    const messages = await Promise.all(result.messages.slice(0, parseInt(opts.max)).map(async (m) => {
...
messagesCmd.command("read <messageId>").alias("get").description("Read a specific message").option("--body", "Include full message body").option("--html", "Show HTML body instead of plain text").option("--structure", "Show message MIME structure (for debugging)").action(async (messageId, opts) => {
  try {
    const gmail = requireAuth();
...
messagesCmd.command("send").description("Send an email").requiredOption("-t, --to <email>", "Recipient email address").requiredOption("-s, --subject <subject>", "Email subject").requiredOption("-b, --body <body>", "Email body").option("--cc <emails>", "CC recipients (comma-separated)").option("--bcc <emails>", "BCC recipients (comma-separated)").option("--html", "Send as HTML email").option("--attachment <path>", "Attach a file (can be specified multiple times)", (val, prev) => prev.concat([val]), []).action(async (opts) => {
  try {
    const gmail = requireAuth();
    const attachments = [];
    if (opts.attachment && opts.attachment.length > 0) {
      for (const attachPath of opts.attachment) {
        const resolvedPath = resolve(attachPath);
        if (!existsSync6(resolvedPath)) {
          error(`Attachment not found: ${attachPath}`);
          process.exit(1);
        }
        const fileData = readFileSync4(resolvedPath);
        attachments.push({
...
messagesCmd.command("reply <messageId>").description("Reply to a message (stays in the same thread)").requiredOption("-b, --body <body>", "Reply body (supports markdown)").option("--cc <emails>", "CC recipients (comma-separated)").option("--html", "Send as HTML email").option("--attachment <path>", "Attach a file (can be specified multiple times)", (val, prev) => prev.concat([val]), []).action(async (messageId, opts) => {
  try {
    const gmail = requireAuth();
    const attachments = [];
    if (opts.attachment && opts.attachment.length > 0) {
...
labelsCmd.command("create <name>").description("Create a new label").option("--bg-color <hex>", "Background color (e.g. #16a765)").option("--text-color <hex>", "Text color (e.g. #ffffff)").action(async (name, opts) => {
  try {
    const gmail = requireAuth();
...
labelsCmd.command("update <labelId>").description("Update a label name and/or color").option("--name <name>", "New label name").option("--bg-color <hex>", "Background color (e.g. #16a765)").option("--text-color <hex>", "Text color (e.g. #ffffff)").action(async (labelId, opts) => {
  try {
...
labelsCmd.command("rename <labelId> <newName>").description("Rename a label").action(async (labelId, newName) => {
  try {
    const gmail = requireAuth();
...
attachmentsCmd.command("download <messageId>").description("Download all attachments from a message").option("-a, --attachment-id <id>", "Download specific attachment by ID").option("-d, --dir <path>", "Directory to save attachments (default: connector storage dir)").action(async (messageId, opts) => {
  try {
    const gmail = requireAuth();
    ...
    if (opts.attachmentId) {
      ...
      const result = await gmail.attachments.download(messageId, attachment.attachmentId, attachment.filename, attachment.mimeType, outputDir);
...
attachmentsCmd.command("bulk-download <query>").description("Bulk-download attachments from messages matching a Gmail search query").option("-o, --output <dir>", "Output directory", "./gmail-attachments").option("--organize-by-date", "Organize files into subdirs by email date (YYYY/MM/)").option("-n, --max <number>", "Maximum messages to process", "100").action(async (query, opts) => {
  try {
    const gmail = requireAuth();
    const outputDir = resolve(opts.output);
    const organizeByDate = !!opts.organizeByDate;
    const maxMessages = parseInt(opts.max);
    info(`Searching for messages matching: ${source_default.cyan(query)}`);
    info(`Output directory: ${source_default.cyan(outputDir)}`);
    if (organizeByDate)
      info("Organizing by date: enabled");
    const allMessageIds = [];
    let pageToken;
    do {
      const result = await gmail.messages.list({
    ...
    for (let i = 0;i < allMessageIds.length; i++) {
      ...
      try {
        ...
        if (organizeByDate) {
          const fullMsg = await gmail.client.get(`/users/me/messages/${messageId}`, { format: "minimal" });
          const msgDate = fullMsg?.internalDate ? new Date(parseInt(fullMsg.internalDate)) : new Date;
          const year = msgDate.getFullYear().toString();
          const month = (msgDate.getMonth() + 1).toString().padStart(2, "0");
          destDir = join6(outputDir, year, month);
        }
        ...
        for (const attachment of attachments) {
          try {
            ...
            let destPath = join6(destDir, finalFilename);
            if (existsSync6(destPath)) {
              const dotIdx = cleanFilename.lastIndexOf(".");
              const suffix = "_" + messageId.slice(-6);
              finalFilename = dotIdx !== -1 ? cleanFilename.slice(0, dotIdx) + suffix + cleanFilename.slice(dotIdx) : cleanFilename + suffix;
              destPath = join6(destDir, finalFilename);
            }
            const buffer = Buffer.from(data.data, "base64url");
            writeFileSync6(destPath, buffer);
            info(`${progress} ${source_default.green("\u2713")} ${finalFilename} (${(buffer.length / 1024).toFixed(1)} KB)`);
            totalDownloaded++;
          } catch (attachErr) {
            warn(`${progress} ${source_default.red("\u2717")} ${attachment.filename}: ${String(attachErr)}`);
            totalErrors++;
          }
        }
      } catch (msgErr) {
        warn(`${progress} ${source_default.yellow("!")} Message ${messageId}: ${String(msgErr)}`);
        totalSkipped++;
      }
    }
    info("");
    ...
    if (totalSkipped > 0)
      info(`  Skipped: ${totalSkipped} message(s) with errors`);
    if (totalErrors > 0)
      info(`  Failed: ${totalErrors} attachment(s)`);
...
bulkCmd.command("label <query>").description("Add or remove labels from messages matching a query").option("-a, --add <labels>", "Labels to add (comma-separated names or IDs)").option("-r, --remove <labels>", "Labels to remove (comma-separated names or IDs)").option("-n, --max <number>", "Maximum messages to process", "100").option("--all", "Process all matching messages (paginates through entire result set)").option("--offset <number>", "Skip first N results before processing", "0").option("--skip-labeled", "Skip messages that already have the target label(s) applied").option("--dry-run", "Preview changes without applying them").option("--batch", "Use Gmail batch API for faster processing (recommended for large batches)").action(async (query, opts) => {
  try {
    ...
    if (opts.all)
      info(`  Mode: process ALL matching messages`);
    if (offset > 0)
      info(`  Offset: skipping first ${offset} results`);
    if (skipIfLabeled)
      info(`  Skipping already-labeled messages`);
    let result;
    if (opts.batch) {
    ...
    if (result.skipped > 0)
      info(`  Skipped (already labeled): ${result.skipped}`);
    if (result.failed > 0) {
