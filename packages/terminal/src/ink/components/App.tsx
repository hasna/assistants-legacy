...
export default class App extends PureComponent<Props, State> {
  ...
  stdinMode: 'readable' | 'data' = process.env.TAKUMI_USE_READABLE_STDIN === '1' ? 'readable' : 'data';
  ...
  override componentDidMount() {
    ...
    if (this.props.stdout.isTTY && !isEnvTruthy(process.env.TAKUMI_ACCESSIBILITY)) {
  ...
  handleSuspend = (): void => {
    ...
    // Emit suspend event for Takumi to handle. Mostly just has a notification
    ...
    const resumeHandler = () => {
      ...
      if (this.props.stdout.isTTY) {
        if (!isEnvTruthy(process.env.TAKUMI_ACCESSIBILITY)) {
          this.props.stdout.write(HIDE_CURSOR);
        }
      ...
      // Emit resume event for Takumi to handle
...
function processKeysInBatch(app: App, items: ParsedInput[], _unused1: undefined, _unused2: undefined): void {
  ...
  for (const item of items) {
    ...
    if (event.didStopImmediatePropagation()) {
      // Legacy useInput handlers still own some keys; don't also run DOM
      // keydown handlers or their default actions after one consumes input.
      continue;
    }
