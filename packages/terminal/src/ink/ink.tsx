...
export default class Ink {
  ...
  exitAlternateScreen(): void {
    ...
    // re-enable mouse (skip if TAKUMI_DISABLE_MOUSE)
  ...
  unmount(error?: Error | number | null): void {
    ...
    // updateContainerSync is preferred (React 19+) but may not exist in all
    // react-reconciler builds. Fall back to the older updateContainer API.
  ...
  private patchStderr(): () => void {
    ...
    const intercept = (chunk: Uint8Array | string, encodingOrCb?: BufferEncoding | ((err?: Error) => void), cb?: (err?: Error) => void): boolean => {
      const callback = (typeof encodingOrCb === 'function' ? encodingOrCb : cb) as
        | ((err?: Error | null) => void)
        | undefined;
